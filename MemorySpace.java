...
public class MemorySpace {
	...
	public int malloc(int length) {
		...
		if (matchBlock != null) {
			int address = matchBlock.block.baseAddress;
			MemoryBlock newbBlock = new MemoryBlock(address, length);
			allocatedList.addLast(newbBlock);

			if (matchBlock.block.length == length) {
				freeList.remove(matchBlock);
			} else {
				matchBlock.block.baseAddress += length;
				matchBlock.block.length -= length;
			}
